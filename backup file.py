...
def generate_legs(side, main_body_sizes):
    ...
    # Leg2 size and position
    leg2_x_size = np.random.uniform(0.15, 0.25)
    leg2_y_size = np.random.uniform(0.15, 0.25)
    leg2_z_size = np.random.uniform(0.05, 0.15)
    leg2_x_pos = leg1_x_pos  # Aligned with leg1 in x
    leg2_z_pos = leg1_z_pos  # Aligned with leg1 in z
    leg2_y_pos = leg1_y_pos + (leg1_y_size + leg2_y_size) if side == 'left' else (leg1_y_pos - (leg1_y_size + leg2_y_size))

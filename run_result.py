...
data = np.loadtxt('data/maxheight_x_0.186_y_0.16_z_0.089.csv', delimiter=',')

height = data[:, 23]
idx_min_height = np.argmin(height)
min_height = height[idx_min_height]
idx_max_height = np.argmax(height)
...
def run_simulation(parameters, filename_suffix):
    ...
    filename = "temp/maxheight_{filename_suffix}.xml"
...
print(f"Generation {idx_max_height}, Height: {max_height}")
